...
function optimise_yaws()


    #region fold initialisation and setup
    base_dir = @__DIR__

    vis_file = joinpath("data/vis_default.yaml")
    settings_file = joinpath("data/REALWF_CONTROLTEST_VEER.yaml")
    vis = Vis(vis_file)
    plt = nothing

    include(joinpath(base_dir, "../../examples/remote_plotting.jl"))
    include(joinpath(base_dir, "optimisationstructs.jl"))
    include(joinpath(base_dir, "functions.jl"))
    ...
    if !@isdefined(tlv)
        tlv = Ref{TaskLocalValue{NamedTuple}}()
    end
    ...
    set_num_optimiser_runs = 1        # 4       #number of automatic restarts for CMA-ES
    set_cmaes_lambda_multiplier = 4    # 4       #multiplier for the default lambda, which is 4 + 3 * log(N), N is dim of problem
    set_iterations = 20                # 50     #number of iterations for CMAES
    ...
    return (result, all_traces, total_time);
...
function postprocessing(result, all_traces, total_time)
base_dir = @__DIR__
    ...
    println()
     include(joinpath(base_dir, "calculate_increase_over_baseline.jl"))  #to calculate the increase over baseline for the optimised case, compared to a baseline case with no yawing
...
result, all_traces, total_time = optimise_yaws();
postprocessing(result, all_traces, total_time);
