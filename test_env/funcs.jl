...
function VEERWAKEMODELBuffers(nRP::Int)::VEERWAKEMODELBuffers
    return VEERWAKEMODELBuffers(
        ...
        Vector{Float64}(undef, nRP),   #u_in_z
...
function compute_wake_effects!(buf::VEERWAKEMODELBuffers, par::Params, views, RP_data)
    ...
    @inbounds for i in 1:nRP                #inbounds for performance                 
        ...
        u_in_z[i] = par.u_hub * shear_modifier[i]
        
        #compute t_hat
        t_hat[i] = (
            -1.44 * u_in_z[i]/ par.u_star * par.R / xi_0_hat[i] * par.CT  
        ...
        sigma_hat_squared[i] = (
            (par.k * par.u_star / (u_in_z[i])  * coords_veered[i,1] + 0.4 * xi_0_hat[i]) *
            (par.k * par.u_star / (u_in_z[i])  * coords_veered[i,1] + 0.4 * xi_0_hat[i] * cos(gamma[i]))
        ) # == (kx + 0.4 xi_0_hat     )(         ) 
        sigma[i] = (par.k * par.u_star / (u_in_z[i])  * coords_veered[i,1] + 0.4 * xi[i])


        #eq 15: velocity deficit
        c[i] = 1.0 - sqrt(max(0.001,      1.0 - par.R^2 * par.CT * cos(gamma[i])^3 / (2.0 * sigma_hat_squared[i]) ))
        du[i] = c[i] * exp(- ((coords_veered[i,2] - y_c[i])^2 + (coords_veered[i,3] - par.z_hub)^2 )  / (2.0*sigma[i]^2)   ) * (u_in_z[i])
        u[i] = u_in_z[i] - du[i]
    end
    #print(sigma)
