...
@btime eff_area = 2.0 * sum(    sqrt.(h_sq) .*  cos.(eff_yaw).^p_p .* dz    )
println("Numerical: ")
println(eff_area)

println()
#analytical approx
@btime eff_area_analytical = pi * R^2 * cosd(yaw)^p_p * (1.0 - (p_p * deg2rad(veer_gradient)^2 * R^2 ) / 8.0 *  (1.0 -(p_p-1.0) * tand(yaw)^2 ) ) 
